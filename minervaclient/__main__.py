...
def parse_register(subp):
    parser = subp.add_parser(
        'register', help='Register for a set of courses', aliases=['reg'], add_help=False)
    parser.add_argument(
        "-t", "--term", help="The term (e.g. FALL2016, 201609, etc.) in which to register for these courses", required=True)
    ...
    parser.add_argument("-A", "--require-all",
                        help="Only register for courses if all of the requested courses are available. NOTE: Only allowed when course codes are given.", action="store_true")
    parser.add_argument("-R", "--require-reg",
                        help="Only register for courses if you can enter the class or the waitlist. NOTE: Only allowed when course codes are given..", action="store_true")
    parser.add_argument("-n", "--no-register",
                        help="Don't actually perform registration.", action="store_true")
    parser.add_argument("-P", "--public-search",
                        help="[BUGGY] Do not log into Minerva to search !!! This option can only be used to register for the waitlist !!!", action="store_true")
    parser.add_argument(
        "-v", "--verbose", help="Show details about the HTTP requests sent to Minerva", action="store_true")
    parser.add_argument(
        "-q", "--quiet", help="[TODO] Only print output for successful registration", action="store_true")
    parser.add_argument("-h", "--help", default=argparse.SUPPRESS,
                        action='help', help="Displays the help message for this subcommand")
    parser.add_argument(
        'course', nargs='+', help="A series of CRNs (e.g. 814 202) or course codes (POLI-244-001 COMP-251-002), but not both.")
    parser.set_defaults(which='register')


def parse_schedule(subp):
    parser = subp.add_parser('schedule', help='Display your course schedule', aliases=[
                             'sched'], add_help=True)
    parser.add_argument(
        "-t", "--term", help="The term (e.g. FALL2016, 201609, etc.) for which to display the schedule", required=True)
    parser.add_argument(
        "-D", "--diff-set", help="[TODO] Monitor the requested report for any changes compared with the previous reports in the diff set")
    parser.add_argument(
        "-r", "--report", help="choose which schedule report to display (see config.py to configure")
    parser.add_argument(
        "-l", "--long", help="Show a detailed report of the schedule", action="store_true")
    parser.add_argument(
        "-s", "--short", help="Show a brief report of the schedule", action="store_true")
    parser.add_argument(
        "-V", "--visual", help="Export a visual course timetable", action="store_true")
    parser.add_argument(
        "-C", "--calendar", help="Export a calendar for the course schedule (or exam schedule if -E is also given)", action="store_true")
    parser.add_argument(
        "-E", "--exams", help="Show the final exam schedule for your courses.", action="store_true")
    parser.add_argument("--conflicts-only",
                        help="Only show course conflicts", action="store_true")
    parser.add_argument(
        "--no-conflicts", help="Don't show course conflicts", action="store_true")
    parser.add_argument(
        "-f", "--format", help="[TODO] Choose text format for the displayed report")
    parser.add_argument(
        "-v", "--verbose", help="Show details about the HTTP requests sent to Minerva", action="store_true")
...
def parse_transcript(subp):
    parser = subp.add_parser(
        'transcript', help='Display your transcript', add_help=False)
    parser.add_argument(
        "-t", "--term", help="The term or terms, separated by commas for which to display the transcript. If no term is specified, records from all terms will be displayed")
    parser.add_argument(
        "-D", "--diff-set", help="[TODO] Monitor the requested report for any changes compared with the previous reports in the diff set")
    parser.add_argument(
        "-r", "--report", help="Choose which transcript report to display (see config.py to configure")
    parser.add_argument(
        "-l", "--long", help="Show a detailed report of the transcript", action="store_true")
    parser.add_argument(
        "-s", "--short", help="Show a brief report of the transcript", action="store_true")
    parser.add_argument(
        "-G", "--grades", help="Include grades in the report", action="store_true")
    parser.add_argument(
        "-C", "--credit", help="Include credit/GPA information in the report", action="store_true")
    parser.add_argument(
        "-S", "--summary", help="Include a summary (program, status, etc). in the report", action="store_true")
    parser.add_argument(
        "-P", "--header", help="Show the transcript header, which includes personal information and previous education, etc.", action="store_true")
    parser.add_argument(
        "-f", "--format", help="[TODO] Choose text format for the displayed report")
    parser.add_argument(
        "-v", "--verbose", help="Show details about the HTTP requests sent to Minerva", action="store_true")
    parser.add_argument("-h", "--help", action='help',
                        help="Displays the help message for this subcommand")
    parser.set_defaults(which='transcript')


def parse_search(subp):
    parser = subp.add_parser(
        'search', help='Search for a course', add_help=False)
    parser.add_argument(
        "-t", "--term", help="The term (e.g. FALL2016, 201609, etc.) in which to register for these courses", required=True)
    parser.add_argument("-C", "--course-type",
                        help="Manually enter the type of section to view (e.g. Lecture, Conference, \"Midterm Exam\", Tutorial)")
    parser.add_argument("-L", "--lecture-only",
                        help="Show only courses that are Lecture types", action="store_true")
    parser.add_argument("-T", "--tutorial-only",
                        help="Show only courses that are Tutorial types", action="store_true")
    parser.add_argument("-A", "--availability-only",
                        help="Show only availability information", action="store_true")
    parser.add_argument(
        "-v", "--verbose", help="Show details about the HTTP requests sent to Minerva", action="store_true")
    parser.add_argument(
        "-d", "--debug", help="Show details about the HTTP requests sent to Minerva", action="store_true")
    parser.add_argument(
        "-q", "--quiet", help="[TODO] Only print output for successful registration", action="store_true")
    parser.add_argument("-h", "--help", default=argparse.SUPPRESS,
                        action='help', help="Displays the help message for this subcommand")
    parser.add_argument(
        'course', nargs='+', help="A series of full course codes or general course codes (POLI-244-001 COMP-251-002 FACC-100) ")
    parser.set_defaults(which='search')


def parse_shell(subp):
    parser = subp.add_parser(
        'shell', help='Start the Minerva Client Interpreter', add_help=False)
    parser.add_argument(
        "-v", "--verbose", help="Show details about the HTTP requests sent to Minerva", action="store_true")
    parser.add_argument(
        "-q", "--quiet", help="[TODO] Only print output for successful registration", action="store_true")
...
def parse_login(subp):
    parser = subp.add_parser(
        'login', help='Enter login credentials for temporary usage', add_help=False)
    parser.add_argument("-u", "--username",
                        help='Provide your Minerva ID number')
    parser.add_argument(
        "-p", "--password", help='Provide your Minerva PIN number \nNOTE: HIGHLY DO NOT RECOMMEND, LEAVES PASSWORD UNPROTECTED.  SHOULD USE NORMAL PROMPT')
    parser.add_argument(
        "-r", "--re-login", help='Repress repeat login message', action="store_true")
    parser.add_argument("-v", "--verbose",
                        help="Show more details", action="store_true")
    parser.add_argument(
        "-q", "--quiet", help="Repress repeat login message", action="store_true")
    parser.add_argument("-h", "--help", default=argparse.SUPPRESS,
                        action='help', help="Displays the help message for this subcommand")
...
def parse_logout(subp):
    parser = subp.add_parser(
        'logout', help='Delete the login credentials, ie. logout', add_help=False)
    parser.add_argument("-v", "--verbose",
                        help="Show more details", action="store_true")
    parser.add_argument(
        "-q", "--quiet", help="Repress messages", action="store_true")
    parser.add_argument("-h", "--help", default=argparse.SUPPRESS,
                        action='help', help="Displays the help message for this subcommand")
...
def exec_reg(args):
    ...
    for course in args.course:
        if codes_given != course_ref_type(course):
            print(
                "\033[1;31mERROR:\033[0m Course codes cannot be combined with CRNs")
    ...
    if args.job:
        ...
        if not courses:
            print(
                "\033[1;32m**** Congratulations, you've gotten into all your courses ****\033[1m")
            sys.exit(0)
    else:
        courses = args.course

    if codes_given:
        data = reg.check_register(term, courses, require_all=args.require_all,
                                  require_reg=args.require_reg, dry_run=args.no_register, public_search=args.public_search)
    else:
        ...
        if args.require_all or args.require_reg:
            ...
            print(
                "\033[1;31mERROR:\033[0m This feature is implemented only when searching by course code")
...
def exec_sched(args):
    ...
    if not args.exams:
        sched.course_details(term, report, visual=args.visual, calendar=args.calendar,
                             conflicts_only=args.conflicts_only, no_conflicts=args.no_conflicts)
    else:
...
def exec_search(args):
    ...
    pub_search.print_search(
        term, courses, cType, avail=args.availability_only, verbose=args.verbose, Debug=args.debug)
...
class CommandWrapper(Cmd):
    def __init__(self, cmd_parent, line_args, command_name, command_function, try_login=False):
        ...
        try:
            args = cmd_parent.ap.parse_args(
                (command_name+" "+line_args).split())
            if args.verbose:
                set_loglevel(True)
