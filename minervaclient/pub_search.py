...
# TODO: why future import standard_library shows error on pylinter (py3)
...
def search(term, course_codes):
    ...
    result = requests.post(
        "https://horizon.mcgill.ca/rm-PBAN1/bwckgens.csv", request)
...
def quick_search(term, course_codes, course_type=""):
    ...
    for course_code in course_codes:  # course_code could be COMP-202 or something like COMP-200-001 or something like COMP-361D1-001
        ...
        if(course_code in courses_obj):
            # if the course_code fits the format of COMP-200-001 or COMP-361D1-001 then just to list and move on
            ...
            full_code = course_code + \
                str(counter).join("-000".rsplit('0'*len(str(counter)), 1))
...
def print_search(term, course_codes, cType, avail=False, verbose=False, Debug=False):
    ...
    for full_code in full_codes:
        ...
        if(avail and not (verbose or Debug)):
            ...
            minerva_output.print(" Capacity: %-4s" %
                                 (str(course['reg']['cap'])), end=' ')
            minerva_output.print(" Seats (remain): %-8s" % (
                str(course['wait']['rem']) + "/" + str(course['wait']['cap'])), end=' ')
            minerva_output.print(" Seats (actual): %-8s" % (
                str(course['wait']['act']) + "/" + str(course['wait']['cap'])), end=' ')
            minerva_output.print(" Waitlist (remain): %-8s" %
                                 (str(course['wl_rem']) + "/" + str(course['wl_cap'])), end=' ')
            minerva_output.print(" Waitlist (actual): %-8s" %
                                 (str(course['wl_act']) + "/" + str(course['wl_cap'])))
        else:
            minerva_output.print(beautify_course_info(
                course, (verbose or Debug)))
...
def beautify_course_info(e, Debug=False):
    ...
    result = [
        ...
        e['type'] + " Instructor: " + e['instructor'] +
        " | Credits: "+str(e['credits']),
        "Capacity: " + capacity +
        " | Seats(remains): " + seats_remain +
        " | Waitlist(remains): " + wait_remain,
        "Seats(actual): " + seats_actual +
        " | Waitlist(actual): " + wait_actual,
        e['location'] + " " + e['days'] + " " +
        e['time'] + " | Period: " + e['date'],
        ""
    ]
    result0 = ""
...
def parse_results(text):
    ...
    field_names = ['crn', 'subject', 'course', 'section', 'type', 'credits', 'title', 'days',
                   'time', 'cap', 'wl_cap', 'wl_act', 'wl_rem', 'instructor', 'date', 'location', 'status']
